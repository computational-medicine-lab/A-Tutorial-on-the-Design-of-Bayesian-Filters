...
for m = 1:M
    ...
    next = [alpha(m + 1) r0(m + 1) r1(m + 1) ve(m + 1) vr(m + 1) rho(m + 1) s0(m + 1) s1(m + 1) vs(m +1)];
    ...
    if mean_dev < tol    
        ...
        fprintf('m = %d\nr0 = %.18f\nr1 = %.18f\nvr = %.18f\n\ns0 = %.18f\ns1 = %.18f\nvs = %.18f\n\nve = %.18f\nrho = %.18f\nalpha = %.18f\n\n', ...
        m + 1, r0(m + 1), r1(m + 1), vr(m + 1), s0(m + 1), s1(m + 1), vs(m + 1), ve(m + 1), rho(m + 1), alpha(m + 1));
...
plot(find(n == 0) - 1, 1.2 * max(p_smth) * ones(length(find(n == 0))), 'ks', 'MarkerFaceColor', 'k', 'MarkerSize', 4);
plot(find(n == 1) - 1, 1.2 * max(p_smth) * ones(length(find(n == 1))), 'gs', 'MarkerFaceColor', 'g', 'MarkerSize', 4);
...
plot(find(I == 0) - 1, (-8) * ones(length(find(I == 0))), 'ks', 'MarkerFaceColor', 'k', 'MarkerSize', 4);
plot(find(I == 1) - 1, (-8) * ones(length(find(I == 1))), 'cs', 'MarkerFaceColor', 'c', 'MarkerSize', 4);
